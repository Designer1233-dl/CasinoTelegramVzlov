...
def ib(text: str, icon_key: str | None = None, **kwargs):
...
def kb(text: str, icon_key: str | None = None, **kwargs):
...
async def start(message: types.Message):
    ...
    await message.answer(f"{pe('casino', '🎰')} Главное меню:", parse_mode="HTML", reply_markup=main_menu(message.from_user.id))
...
async def back(callback: types.CallbackQuery):
    await callback.message.edit_text(f"{pe('casino', '🎰')} Главное меню:", parse_mode="HTML", reply_markup=main_menu(callback.from_user.id))
...
async def save_bet(message: types.Message, state: FSMContext):
    amount = parse_positive_float(message.text, MIN_BET)
    if amount is None:
        await message.answer(f"{pe('money', '💵')} Введите ставку числом от {money(MIN_BET)}.")
        return
...
async def choose_game(callback: types.CallbackQuery):
    ...
    await play_game_round(callback.message, callback.from_user.id, game_key, amount)


async def play_game_round(message: types.Message, user_id: int, game_key: str, amount: float):
    game = GAMES[game_key]
    update_balance(user_id, -amount)
    await message.answer(f"{pe('dice', '🎲')} Бросаю реальные кубики Telegram...", parse_mode="HTML")
    ...
    await message.answer(result, parse_mode="HTML", reply_markup=main_menu(user_id))
...
async def admin_createcheck_callback(callback: types.CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        return
    await callback.answer()
    await callback.message.answer(f"{pe('gift', '🎁')} Введите сумму чека:", parse_mode="HTML")
...
async def create_check(message: types.Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    await message.answer(f"{pe('gift', '🎁')} Введите сумму чека:", parse_mode="HTML")
    await state.set_state(AdminCheckState.amount)
